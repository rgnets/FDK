...
class Issue with _$Issue {
  ...
  /// Room-level coverage speed test that ran but came in below the configured
  /// threshold. Unlike the per-AP compliance speed-test issue, this is a
  /// room-level result (no device) so it carries `roomId`, not `deviceId`.
  factory Issue.coverageSpeedTestFailed({
    ...
    final metrics = (downloadMbps != null || uploadMbps != null)
        ? ' (↓ ${downloadMbps?.toStringAsFixed(1) ?? "N/A"} / '
            '↑ ${uploadMbps?.toStringAsFixed(1) ?? "N/A"} Mbps)'
        : '';
    return Issue(
      id: 'coverage_speed_test_${resultId ?? roomId}',
      code: 'COVERAGE_SPEED_TEST_FAILED',
      title: 'Coverage Speed Test Below Threshold',
      description: '$testName did not meet the required speed$metrics',
