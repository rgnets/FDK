...
class DevicesNotifier extends _$DevicesNotifier {
  ...
  Future<List<Device>> build() async {
    ...
    try {
      ...
      final cacheKey = DeviceFieldSets.getCacheKey('devices_list', DeviceFieldSets.listFields);
      final devices = await _cacheManager.get<List<Device>>(
        ...
        fetcher: () async {
          ...
          final result = await getDevices(
            GetDevicesParams(fields: DeviceFieldSets.listFields),
          );
          
          return result.fold(
            (failure) {
              _logger.e('DevicesProvider: Failed to load devices - ${failure.message}');
              throw Exception(failure.message);
            },
            (devices) {
              if (LoggerConfig.isVerboseLoggingEnabled) {
                _logger.i('DevicesProvider: Successfully loaded ${devices.length} devices with ${DeviceFieldSets.listFields.length} fields');
              }
              return devices;
            },
          );
      ...
      _logger.e('DevicesProvider: Exception in build(): $e', error: e, stackTrace: stack);
      rethrow;
    }
  }
  ...
  Future<void> userRefresh() async {
    ...
    try {
      ...
      _logger.e('DevicesProvider: User refresh failed: $e', error: e, stackTrace: stack);
  ...
  Future<void> silentRefresh({String? context}) async {
    if (LoggerConfig.isVerboseLoggingEnabled) {
      _logger.i('DevicesProvider: Silent background refresh (context: $context)');
    }
    ...
    try {
      ...
      // For list view or default, use refresh fields for minimal data
      if (context == null || context == 'list') {
        fields = DeviceFieldSets.refreshFields;
        cacheKey = DeviceFieldSets.getCacheKey('devices_list', fields);
      } else {
        // For other contexts, use list fields
  ...
  void _startBackgroundRefresh() {
    _refreshManager.startSequentialRefresh(() => silentRefresh(context: 'list'));
  }
  ...
  Future<void> rebootDevice(String deviceId) async {
    ...
    try {
      ...
      _logger.e('DevicesProvider: Exception during reboot: $e', error: e, stackTrace: stack);
      rethrow;
    }
  }
}
...
class DeviceNotifier extends _$DeviceNotifier {
  ...
  Future<void> refresh() async {
    ...
    final result = await getDevice(
      GetDeviceParams(
        id: deviceId,
        fields: DeviceFieldSets.detailFields, // Refresh with all fields for detail view
      ),
    );
  ...
  return devices.when(
    data: (deviceList) {
      return deviceList.where((device) {
        ...
        final matchesStatus = filters.status == null || device.status == filters.status;
