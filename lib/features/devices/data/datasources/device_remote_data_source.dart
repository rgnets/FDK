...
class DeviceRemoteDataSourceImpl implements DeviceRemoteDataSource {
    ...
    try {
      ...
      if (response.data is List) {
        ...
        if (data['results'] != null && data['results'] is List) {
          ...
          _logger.w('No recognized data key found in response from $endpoint. Keys: ${data.keys}');
          return [];
        }
      } else {
        _logger.e('Unexpected response type from $endpoint: ${response.data.runtimeType}');
        return [];
      }
  ...
  Future<List<DeviceModel>> getDevices({
    ...
    try {
      ...
      _logger
        ..i('📡 DEVICE_REMOTE_DATA_SOURCE: getDevices() CALL #$callId at ${DateTime.now().toIso8601String()}')
        ..i('DeviceRemoteDataSource: Getting devices with fields: ${fields?.join(',')}')
        ..i('🔍 DEBUG: Call stack trace for getDevices() - Call #$callId');
      final stopwatch = Stopwatch()..start();
      
      // Fetch all device types in parallel with retry logic
      _logger
        ..d('DEVICE_REMOTE_DATA_SOURCE: Starting parallel fetch of device types with retry')
      ...
      stopwatch.stop();
      _logger.i('Total ${allDevices.length} devices collected in ${stopwatch.elapsedMilliseconds}ms');
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (results.isNotEmpty) {
          _logger.i('Successfully fetched ${results.length} $type on attempt $attempt');
          return results;
        }
        ...
        if (attempt == maxRetries) {
          _logger.e('All $maxRetries attempts failed for $type - returning empty list');
          return [];
        }
  ...
  Future<List<DeviceModel>> _fetchDeviceType(
    ...
    try {
      _logger
        ..d('DEVICE_REMOTE_DATA_SOURCE: Fetching $type with fields: ${fields?.join(',')}')
        ..d('Fetching $type')
        ..i('🔍 _fetchDeviceType: Starting fetch for $type at ${DateTime.now().toIso8601String()}');
      ...
      _logger
        ...
        ..i('🔍 _fetchDeviceType: Completed fetch for $type - got ${results.length} items');
      
      // Special debugging for switches
      if (type == 'switch_devices') {
        _logger.i('🔍 SWITCH_DEVICES DEBUG: Raw API returned ${results.length} switches');
        for (var i = 0; i < results.length; i++) {
          final raw = results[i];
          _logger.i('  Raw switch ${i + 1}: ID=${raw['id']}, Name=${raw['name'] ?? raw['nickname']}');
        }
      }
      
      // Debug for WLAN devices
      if (type == 'wlan_devices') {
        _logger.i('🔍 WLAN_DEVICES DEBUG: Raw API returned ${results.length} WLAN devices');
        for (var i = 0; i < results.length; i++) {
          final raw = results[i];
          _logger.i('  Raw WLAN ${i + 1}: ID=${raw['id']}, Name=${raw['name']}, Device=${raw['device']}');
        }
      }
        ...
        switch (type) {
          case 'access_points':
            return DeviceModel.fromJson({
              'id': 'ap_${deviceMap['id']?.toString() ?? ''}',  // Prefixed to avoid ID collisions
          ...
          case 'media_converters':
            return DeviceModel.fromJson({
              'id': 'ont_${deviceMap['id']?.toString() ?? ''}',  // Prefixed to avoid ID collisions
          ...
          case 'switch_devices':
            return DeviceModel.fromJson({
              'id': 'sw_${deviceMap['id']?.toString() ?? ''}',  // Prefixed to avoid ID collisions
              'name': deviceMap['name'] ?? deviceMap['nickname'] ?? 'Switch-${deviceMap['id']}',
              'type': 'switch',
              ...
              'serial_number': deviceMap['serial_number'] ?? '',
              'location': _extractLocation(deviceMap),
              'last_seen': deviceMap['last_config_sync_at'] ?? deviceMap['updated_at'],
              'metadata': deviceMap,
            });
            
          case 'wlan_devices':
            return DeviceModel.fromJson({
              'id': 'wlan_${deviceMap['id']?.toString() ?? ''}',  // Prefixed to avoid ID collisions
              'name': deviceMap['name'] ?? 'WLAN-${deviceMap['id']}',
              'type': 'wlan_controller',
              'status': deviceMap['online'] == true ? 'online' : 'offline',
              'mac_address': deviceMap['mac'] ?? deviceMap['mac_address'] ?? '',
              'ip_address': deviceMap['host'] ?? deviceMap['ip'] ?? deviceMap['ip_address'] ?? '',
              'model': deviceMap['model'] ?? deviceMap['device'] ?? '',
              'serial_number': deviceMap['serial_number'] ?? '',
              'location': _extractLocation(deviceMap),
  ...
  String _determineStatus(Map<String, dynamic> device) {
    ...
    if (device['last_seen'] != null || device['updated_at'] != null) {
      try {
        final lastSeenStr = (device['last_seen'] ?? device['updated_at']).toString();
  ...
  Future<DeviceModel> getDevice(
    ...
    try {
      ...
      try {
        final response = await apiService.get<Map<String, dynamic>>('/api/devices/$id.json');
        if (response.data != null) {
        ...
        try {
          final response = await apiService.get<Map<String, dynamic>>('/api/switch_devices/$id.json');
          if (response.data != null) {
            ...
            return DeviceModel.fromJson({
              ...
              'type': 'switch',
              'status': swMap['online'] == true ? 'online' : 'offline',
          ...
          // Try access points
          try {
            final response = await apiService.get<Map<String, dynamic>>('/api/access_points/$id.json');
            if (response.data != null) {
            ...
            try {
              final response = await apiService.get<Map<String, dynamic>>('/api/media_converters/$id.json');
              if (response.data != null) {
