...
class ConnectionDetailsDialog extends ConsumerStatefulWidget {
  ...
  static Future<void> show(BuildContext context) {
    return showDialog<void>(context: context, builder: (context) => const ConnectionDetailsDialog());
  }

  @override
  ConsumerState<ConnectionDetailsDialog> createState() => _ConnectionDetailsDialogState();
}

class _ConnectionDetailsDialogState extends ConsumerState<ConnectionDetailsDialog> {
  Timer? _timer;
  ...
  String _getNextSyncCountdown() {
    ...
    final secondsSinceLastSync = DateTime.now().difference(_lastSyncTime).inSeconds;
    ...
    return AlertDialog(
      ...
      content: Container(
        ...
        children: [
          _buildDetailRow(
            'Status',
            auth.maybeWhen(authenticated: (_) => 'Connected', orElse: () => 'Disconnected'),
            auth.maybeWhen(authenticated: (_) => Colors.green, orElse: () => Colors.red),
          ),
          const SizedBox(height: 8),
          _buildDetailRow(
            'Server',
            auth.maybeWhen(authenticated: (user) => user.apiUrl, orElse: () => 'Not configured'),
            null,
          ),
          const SizedBox(height: 8),
          _buildDetailRow(
            'Username',
            auth.maybeWhen(authenticated: (user) => user.username, orElse: () => 'Not configured'),
            null,
          ),
          const SizedBox(height: 8),
          ...
          _buildDetailRow(
            'Response Time',
            '${_responseTimeMs}ms',
            _responseTimeMs < 100 ? Colors.green : _responseTimeMs < 500 ? Colors.orange : Colors.red,
          ),
          const SizedBox(height: 8),
          _buildDetailRow(
            'Last Sync',
          ...
          _buildDetailRow(
            'Next Sync',
            _getNextSyncCountdown(),
            Colors.blue,
          ),
          if (EnvironmentConfig.environment != Environment.production) ...[
            const SizedBox(height: 8),
            _buildDetailRow('Env', EnvironmentConfig.name.toUpperCase(), 
              EnvironmentConfig.isDevelopment ? Colors.green : Colors.orange),
            const SizedBox(height: 8),
            _buildDetailRow('Mock Data', EnvironmentConfig.useSyntheticData ? 'Yes' : 'No', null),
          ],
        ],
        ),
      ),
      actions: [TextButton(onPressed: () => Navigator.of(context).pop(), child: const Text('Close', style: TextStyle(color: Colors.blue)))],
    );
  }
